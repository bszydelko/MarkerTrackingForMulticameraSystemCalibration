...
Point BlobTracker::predict()
{
    ...
    if (numPositions == 0) {
        ...
        int sumOfXChanges = ((detectedPositions[2].x - detectedPositions[1].x) * 2) +
            ((detectedPositions[1].x - detectedPositions[0].x) * 1);

        int deltaX = (int)std::round((float)sumOfXChanges / 3.0);

        int sumOfYChanges = ((detectedPositions[2].y - detectedPositions[1].y) * 2) +
            ((detectedPositions[1].y - detectedPositions[0].y) * 1);
        ...
        int sumOfXChanges = ((detectedPositions[3].x - detectedPositions[2].x) * 3) +
            ((detectedPositions[2].x - detectedPositions[1].x) * 2) +
            ((detectedPositions[1].x - detectedPositions[0].x) * 1);

        int deltaX = (int)std::round((float)sumOfXChanges / 6.0);

        int sumOfYChanges = ((detectedPositions[3].y - detectedPositions[2].y) * 3) +
            ((detectedPositions[2].y - detectedPositions[1].y) * 2) +
            ((detectedPositions[1].y - detectedPositions[0].y) * 1);
...
Rect2d BlobTracker::window(int sizeFactor = 1)
{
    ...
    if (_window.width > seqResolution.width)
        _window.width = seqResolution.width;
    if (_window.height > seqResolution.height)
        _window.height = seqResolution.height;

    if (_window.x < 0)
        _window.x = 0;
    if (_window.x + _window.width > seqResolution.width)
        _window.x = seqResolution.width - _window.width;
    if (_window.y < 0)
        _window.y = 0;
    if (_window.y + _window.height > seqResolution.height)
        _window.y = seqResolution.height - _window.height;
...
void BlobTracker::update(const Point2d& _pos)
{
    ...
    if (_pos != predicted)
	    detectedPositions.push_back(_pos);
}
...
double BlobTracker::avgDist(int n)
{
        ...
        {
            sum += (std::sqrt(
                std::pow((*it).x - (*(it + 1)).x, 2) +
                std::pow((*it).y - (*(it + 1)).y, 2)));
